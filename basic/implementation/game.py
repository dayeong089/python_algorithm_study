...
>> 구현 1 : 각 경우의 수를 고려하여 코드를 작성(방향마다 코드를 따로 작성해줌) + 캐릭터가 맵 밖으로 나가는 경우도 고려해줌
>> 구현 2 : dx, dy 리스트를 사용해 방향을 설정해주는 방법으로 코드를 작성 
''''
...
while True:
    ...
    check = 0
    for _ in range(4):
        if d == 0 and b-1 >= 0 and lst[a][b-1] == 0:
            b -= 1
            d = 3
            check = 1
        elif d == 1 and a-1 >= 0 and lst[a-1][b] == 0:
            a -= 1
            d = 0
            check = 1
        elif d == 2 and a+1 < m and lst[a+1][b] == 0:
            a += 1
            d = 1
            check = 1
        elif d == 3 and b+1 < n and lst[a][b+1] == 0:
            b += 1
            d = 2
            check = 1
        if check == 1:
    ...
    if check == 1:
        ...
        if d == 0:
            if a+1 < m and lst[a+1][b] == 0:
                a += 1
                check = 1
        elif d == 1:
            if b-1 >= 0 and lst[a][b-1] == 0:
                b -= 1
                check = 1
        elif d == 2:
            if a-1 >= 0 and lst[a-1][b] == 0:
                a -= 1
                check = 1
        elif d == 3:
            if b+1 < n and lst[a][b+1] == 0:
                b += 1
                check = 1
